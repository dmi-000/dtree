...
template<
  ...
  class Allocator = shared_memory_allocator<typename std::map<shared_ptr<Key>, shared_ptr<T>>::value_type>
  >
using shared_map = std::map< shared_ptr<Key>, shared_ptr<T>, Compare, Allocator>;

#if 0
#if 0
typedef map < shared_ptr<shared_string>, shared_ptr<shared_string>, key_less,
	      shared_memory_allocator<map<shared_ptr<shared_string>, shared_ptr<shared_string>>::value_type> > Map;
#else
typedef shared_map<shared_string,shared_string, key_less> Map;
#endif

#else
#if 0
typedef map < shared_ptr<shared_string>, shared_ptr<std::string>, key_less,
	      shared_memory_allocator<map<shared_ptr<shared_string>, shared_ptr<std::string>>::value_type> > Map;
#else
typedef shared_map<shared_string,std::string, key_less> Map;
#endif
#endif
...
#if 0
#if 0
	  im = m->insert(im, make_pair( shared_ptr<shared_string>(p, d, a), shared_ptr<shared_string>(p, d, a) ) );
#else
	  (*m)[shared_ptr<shared_string>(p, d, a)]=shared_ptr<shared_string>(p, d, a);
#endif
#else
	  //string *ps= new (shared_memory_allocator<Map>::rebind<std::string>::other(a).allocate(1, (std::string*) 0)) std::string(a);
	  //
	  std::string *ps= new (shared_memory_allocator<Map>::rebind<std::string>::other(a).allocate(1, (std::string*) 0)) std::string(a);
	  auto sp=make_shared<std::string>(line);
	  (*m)[shared_ptr<shared_string>(p, d, a)]=sp;
#endif
	}
