...
public:
  ...
  bool operator()(shared_ptr<char> const &x, shared_ptr<char> const &y) const { return character_set_t(dictorder).key_less(x.get(), y.get()); }
};

typedef multimap < shared_ptr<char>, pos_type, key_less,
	shared_memory_allocator<multimap<shared_ptr<char>, pos_type>::value_type> > Map;
...
void run(char cmd, const char *arg, istream &in = cin) {
  ...
  case 't':
	...
	for (pos_type pos; (pos = fin.tellg()), getline(fin, line); ) {
	  char *p = reinterpret_cast<char*>(shared_memory_allocator<Map>::rebind<char*>::other(a).allocate(sizeof(char) * line.length(), (char*) 0));
	  strncpy(p, line.c_str(), line.length());
	  im = m->insert(im, make_pair(shared_ptr<char>(p, d, a), pos));
	}
