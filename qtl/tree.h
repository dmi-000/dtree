...
class operation{
  ...
  operand_t eval(const std::vector<operand_t> &v,const S_&... s_)const{
	  ...
	  // if( o == op::greater ){
	     ...
	     /*
./qtl/tree.h:271:86: error: invalid operands to binary expression ('basic_ostream<char>' and 'const std::vector<operand_t>' (aka 'const vector<qtl::basic_interval<lex::scalar, void>>'))
           TRACE( std::cout << "optable.at(" << (int)o << ").eval)(" << *this << "," << v << ",s_...)" << '\n'; )
                  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ^  ~
...
#define Xinfix(n,o)  T(n,o, return ( (operand_t)(v[0]).o((operand_t)(v[1]))); )
...
template<typename Vp=std::vector<ps>>
static inline ps stri_infix(const operation &o,const std::string& s,const Vp &v){
  return ps(par(o.precedence(),v[0]) + " " + s +" " + par(o.precedence(),v[1]),o.precedence());
}
...
#undef X
  ...
  ps stringify(const std::vector<ps>&v)const{
    NOTRACE( std::cerr << __PRETTY_FUNCTION__ << '\n'; )	\
      if( 0 <= (int)o && (int)o < optable.size() ){
         return (*optable.at(o).stringify)(*this,v);
  }else{
   using namespace std::string_literals;
   return ps(" invalid expr "s + std::to_string((int)o),0);
  }
