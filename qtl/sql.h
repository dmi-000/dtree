...
static auto const insert_clause=
  (no_case[ lit("INSERT") > lit("INTO") ] > name_rule >> -no_case[ lit("VALUES")] >> -lit("(") >> list_rule > -lit(')') )
  [ ([](auto& ctx){
...
static auto const select_clause=
  ...
  [ ([](auto& ctx){
	 ...
	 NOTRACE( std::cout << file << '\n'; );
         ...
         auto v=at_c<2>(_attr(ctx))?file[from][at_c<2>(_attr(ctx))->bind( symtab[from])]:file[ from ];
#if 0
         auto v=file[ from ];
