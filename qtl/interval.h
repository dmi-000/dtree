  ...
  basic_interval ＾(const basic_interval &b,const basic_interval &m)const{
    ...
    return *this;
     ...
     std::string matcher(){
       std::string r;
       int i;
       std::string L=l().value();
       std::string U=u().value();
       for( i=0;i<L.size()&&i<U.size()&&L[i]==U[i] ;++i ){
	   switch( L[i] ){
   	   case '\\':case '.': case '?':case'*': case'+': case'[': case ']': case '(': case ')':{ r.push_back('\\'); };break;
	   default: break;
	   }
	   r.push_back(L[i]);
       }
	 ...
	 char l=L[i];
         switch( l ){
	 case '\\': case'[': case']':case'-':case'^':{ r.push_back('\\'); };break;
	 default: break;
	 }
	 r.push_back(l);
       }else{
	 ...
	 r.push_back(u);
 	 ...
 	 r.append("\\xff");
       ...
       r.append("].*");
       TRACE( std::cout << __FUNCTION__ << "=" << r << "\n"; )
       return r;
     }
     basic_interval match(const std::string &s){
       if( !is_point() ){
	 return re_and(matcher(),s)? kleen::U : kleen::F;
       }
       auto r=std::basic_regex<char>(s);
	 std::smatch m;
	 if( std::regex_match(s,m,r) ){
#define TR(x)  std::cout << #x "="  << x << '\n'
	   TRACE( TR(m.size()); TR(m.prefix().str()); TR(m[0].str()); )
#undef TR
	   return kleen::T;
	 }else{
	   return kleen::F;
     ...
     basic_interval RLIKE(const basic_interval &r){
       if( !r.is_point() ){
	 return kleen::U;
       }
       return match((std::string)r.l().value());
     }
